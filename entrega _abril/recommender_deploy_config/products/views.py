...
def recomendaciones_xti(usuario):
    ...
    for cada_c in configuracion_layout:
        ...
        if algoritmo == "calificados":
            ...
            productos_usuarios_similares = recomendaciones_contenido.recomendaciones_usuarios_similares(usuario, longitud)
            ...
            recomendaciones_predictivas = recomendaciones_filtrado_colaborativo.recomendaciones_hibridas(productos_fc, longitud)
...
def index_busqueda(request):
    ...
    if request.GET.get('busqueda_main') is not None: 
        texto_busqueda = request.GET.get('busqueda_main').encode("utf-8")
