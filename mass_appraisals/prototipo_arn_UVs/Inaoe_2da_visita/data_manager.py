	...
	for row in cursor:
		...
		for each_field in row:
			if is_num(row[each_field]):
				...
				print "campo: ", each_field, " no es numerico, este avaluo no se agregara"
...
def filter_appraisals(data, filters): 
	...
	for each_appraisal in data:
		...
		if verify and float( each_appraisal["VALOR_FISICO_TERRENO"]) > 200000 and float(each_appraisal["VALOR_FISICO_TERRENO_M2"]) > 10000 and float(each_appraisal["VALOR_FISICO_CONSTRUCCION"]) > 200000 and float(each_appraisal["IMPORTE_VALOR_CONCLUIDO"]) > 400000:
...
def normalise(each_appraisal):
	...
	for each_field in each_appraisal:
		...
		if str(each_field).startswith("DISTANCIA"):
			...
			if each_appraisal[each_field] < 1 or each_appraisal[each_field] >= 4000:
				each_appraisal[each_field] = 1.0
			else:
				each_appraisal[each_field] = log(each_appraisal[each_field])/8.3

	each_appraisal["codigo_postal_ubicacion_inmueble"] = str(each_appraisal["codigo_postal_ubicacion_inmueble"])
	each_appraisal["CVE_CLASE_INMUEBLE"] = float(each_appraisal["CVE_CLASE_INMUEBLE"])/7
