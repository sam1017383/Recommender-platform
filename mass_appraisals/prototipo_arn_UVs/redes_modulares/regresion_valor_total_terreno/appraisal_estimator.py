...
report_fields_valor_terreno = ["Fecha_hora_experimento",
			"Clave_experimento",
...
report_file_name = "report_ANN_UVs_go4it_oct_2016_valor_total_terreno"
...
if not os.path.isfile(f):
	csvfile = open(f, 'a')
	writer = csv.DictWriter(csvfile, fieldnames=report_fields_valor_terreno)
	writer.writeheader()
else:
	csvfile = open(f, 'a')
	writer = csv.DictWriter(csvfile, fieldnames=report_fields_valor_terreno)
...
for hidden_size in [6,8,12]:
	...
	for each_fold in range(k):
		...
		training_stats, trained_network = ann_engine.train_ann(training_set, input_terreno_fields, ["TOTAL_VALOR_TERRENO"], hidden_size, epochs)
		print "TRAINING STATS:", training_stats 
		# activacion de la red con los avaluos de prueba
		activations_stats = ann_engine.activate_network(trained_network, test_set , input_terreno_fields, ["TOTAL_VALOR_TERRENO"])
